  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
            ...
            <Card className="border-0 shadow-none h-full flex flex-col">
              <CardHeader className="bg-brand-gradient text-white p-4 flex flex-row justify-between items-center space-y-0 relative overflow-hidden">
                ...
                <div className="flex items-center gap-3 z-10">
                  <div className="relative">
                    ...
                    <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-[#0F1B2D] rounded-full"></div>
                  </div>
                  <div>
              ...
              <CardFooter className="p-3 bg-white border-t border-gray-100">
                <div className="flex w-full items-center gap-2">
                  <Input placeholder="Escribe tu duda..." value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyPress} className="focus-visible:ring-[#0F1B2D] border-gray-200 bg-gray-50 rounded-full px-4" disabled={chatMutation.isPending} />
                  <Button size="icon" onClick={handleSendMessage} disabled={chatMutation.isPending || !inputValue.trim()} className="bg-[var(--color-brand-yellow)] text-[#0F1B2D] hover:bg-yellow-400 rounded-full w-10 h-10 shadow-sm shrink-0">
      ...
      <motion.button
        ...
        className={cn("h-16 w-16 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 border-4 p-0 overflow-hidden", isOpen ? "bg-gray-200 border-gray-100 text-gray-600" : "bg-[#0F1B2D] border-[var(--color-brand-yellow)] text-white hover:shadow-[0_0_30px_rgba(255,195,0,0.4)]")}
      >
