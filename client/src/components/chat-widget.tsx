...
export function ChatWidget() {
  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    ...
    return saved
      ...
      : [
          {
            role: "model",
            text: "¡Hola! Soy Volt ⚡. Estoy aquí para ayudarte a ahorrar en tu factura. ¿Cómo te puedo ayudar hoy?",
          },
        ];
  });
  ...
  const clearChat = () => {
    ...
    setMessages([
      {
        role: "model",
        text: "Chat reiniciado 🗑️. ¿En qué puedo ayudarte hoy?",
      },
    ]);
  };
  ...
  const chatMutation = useMutation({
    ...
    onError: () => {
      setMessages((prev) => [
        ...prev,
        { role: "model", text: "Ups, algo falló. Inténtalo de nuevo." },
      ]);
    },
  });
  ...
  // ANIMACIÓN DE VIDA (Respiración + Flote)
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
            ...
            <Card className="border-0 shadow-none h-full flex flex-col">
              ...
              <CardHeader className="bg-brand-gradient text-white p-4 flex flex-row justify-between items-center space-y-0 relative overflow-hidden">
                ...
                <div className="flex items-center gap-3 z-10">
                  ...
                  <div>
                    <CardTitle className="text-lg font-bold font-heading tracking-wide">
                      Asistente Volt
                    </CardTitle>
                ...
                <div className="flex gap-1 z-10">
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8 text-white hover:text-[var(--color-brand-yellow)] hover:bg-white/10 rounded-full transition-colors"
                    onClick={clearChat}
                ...
                <ScrollArea className="h-[350px] p-4">
                  <div className="flex flex-col gap-4">
                    {messages.map((msg, index) => (
                      ...
                      >
                        {/* AVATAR ANIMADO (Solo para el Bot) */}
                        {msg.role === "model" && (
                          <motion.div
                        ...
                        <div
                          className={cn(
                            "flex flex-col gap-1 rounded-2xl px-4 py-2.5 text-sm shadow-sm max-w-[80%] break-words whitespace-pre-wrap",
                            msg.role === "user"
                      ...
                      <div className="flex w-full justify-start">
                        ...
                        >
                          <img
                            src={voltAvatar}
                            alt="Volt"
                            className="w-8 h-8 rounded-full border border-gray-200 opacity-70"
                          />
                        </motion.div>
              ...
              <CardFooter className="p-3 bg-white border-t border-gray-100">
                <div className="flex w-full items-center gap-2">
                  <Input
                    ...
                    className="focus-visible:ring-[#002782] border-gray-200 bg-gray-50 rounded-full px-4"
                  ...
                  <Button
                    ...
                    className="bg-[var(--color-brand-yellow)] text-[#002782] hover:bg-yellow-400 rounded-full w-10 h-10 shadow-sm shrink-0"
                  >
      ...
      <motion.button
        ...
        className={cn(
          "h-16 w-16 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 border-4 p-0 overflow-hidden",
          isOpen
            ? "bg-gray-200 border-gray-100 text-gray-600"
            : "bg-[#002782] border-[var(--color-brand-yellow)] text-white hover:shadow-[0_0_30px_rgba(255,195,0,0.4)]",
        )}
      >
        {isOpen ? (
          <X className="h-7 w-7" />
